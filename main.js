...
log.transports.file.resolvePath = () => path.join('D:\samvaad-electron-app', 'logs/main.log');
 ...
 */
let mainWindow = null;
let newMainWindow = null;
let isConferenceStarted = false;
...
/**
 * Opens second window with index.html(Jitsi Meet is loaded in iframe there).
 */
function _newcreateJitsiMeetWindow() {
    // Application menu.
    setApplicationMenu();
    ...
    newMainWindow = new BrowserWindow(options);
    windowState.manage(newMainWindow);
    newMainWindow.loadURL(indexURL);
    ...
    initPopupsConfigurationMain(newMainWindow);
    setupAlwaysOnTopMain(newMainWindow, null, windowOpenHandler);
    setupPowerMonitorMain(newMainWindow);
    // setupScreenSharingMain(newMainWindow, config.default.appName, pkgJson.build.appId);
    if (ENABLE_REMOTE_CONTROL) {
        new RemoteControlMain(newMainWindow); // eslint-disable-line no-new
    }
...
function handleProtocolCall(fullProtocolCall) {
    ...
    //if the conference is started alredy in one window, then don't open another window
    ...
    //for second instance
    if (rendererReady && mainWindow != null && newMainWindow === null) {
        _newcreateJitsiMeetWindow();
    }
}
...
app.on('second-instance', (event, commandLine) => {
     ...
     * existing window.
     */
    if (mainWindow && newMainWindow === null) {
    ...
    if (mainWindow != null && newMainWindow) {
        newMainWindow.isMinimized() && newMainWindow.restore();
        newMainWindow.focus();
...
ipcMain.on('renderer-ready', () => {
    rendererReady = true;
    if (protocolDataForFrontApp && newMainWindow != null) {
        newMainWindow
            .webContents
            .send('protocol-data-msg', protocolDataForFrontApp);
    }
